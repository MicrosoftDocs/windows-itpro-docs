...
Until now, Windows logon only supported the use of identities federated to ADFS or other providers that support the WS-Fed protocol. We are introducing “web sign-in,” a new way of signing into your Windows PC. Web Sign-in enables Windows logon support for credentials not available on Windows (e.g. Temporary Access Pass). Going forward, Web sign-in will be restricted to only support Azure AD temporary access pass. 
