...
2. Extract the contents of the .zip file to a shared, read-only location that can be accessed by the network administrators who will deploy the package. You should have a folder called *WindowsDefenderATPOnboardingPackage* and the file *WindowsDefenderATPOnboardingScript.cmd*.
