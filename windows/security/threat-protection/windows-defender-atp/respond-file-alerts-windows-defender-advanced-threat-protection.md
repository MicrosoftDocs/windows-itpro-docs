...
4. If the sample collection policy is not configured, then the default behavior is to allow sample collection. If the sample collection policy is configured, then verify the policy is configured to allow collection of samples before submitting the file again. When sample collection is configured, then check the following registry value:
```
...
Value: 
  ...
  1 - allows sharing of all file types from this machine. The default value in the case where the registry entry doesn’t exist.
```
