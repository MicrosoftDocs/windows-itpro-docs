...
It is currently possible to set a convenience PIN on Azure Active Directory Joined or Hybrid Active Directory Joined devices. Convenience PIN is not supported for Azure Active Directory user accounts. For instance, if you try to change a convenience PIN on Azure AD Registered device, it fails. It is only supported for on-premises only Domain Joined users and local account users.
