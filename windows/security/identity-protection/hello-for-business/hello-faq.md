...
Yes. It is currently possible to set a convience PIN on an Azure Active Directory Joined or Hybrid Active Directory Joined devices. Convience PIN is not supported for Azure Active Directory user accounts. It is only supported for on-premises only Domain Joined users and local account users. You can set Windows Hello for Business to **Not Configured** in Intune to use this feature.
