...
Run the following command to ensure a generic SRV record for a 2016 domain controller can be discovered: 
nltest /dsgetdc:<onpremuserdomainFQDN> /ds_10
After the provider locates an active 2016 domain controller, the provider uses the private key to sign the Kerberos pre-authentication data.|
