...
The EnrollmentStatusTracking configuration service provider is used by the blocking Enrollment Status Page (ESP) to track Win32 app installations through Sidecar, SCCM, or any other client agent used to install apps. It sends the installation status of both the provider and the Win32 apps back to the ESP, which is displayed on the ESP UI.

The following diagram shows the EnrollmentStatusTracking configuration service provider in tree format.
...
Required. Root node for the CSP. Provides the settings to communicate the policies that the ESP should block. Using these settings, policy providers register themselves and the set of policies that need to be tracked. The ESP includes the counts of these policy sets in the status message to the user, and blocks progress on that page until all policies are provisioned. The policy provider is expected to drive the status updates by updating the appropriate node values, which is then reflected in the ESP status message.

Scope is permanent. Supported operation is Get.

<a href="" id="setup"></a>**Setup**  
Required. Provides the settings that ESP reads during the Account Setup phase. Policy providers use this node to communicate progress state back to the ESP, which is then displayed to the user through progress message updates.

Scope is permanent. Supported operation is Get.

<a href="" id="apps"></a>**Apps**  
Required. Policy providers use this node to communicate with ESP about which app installations it should block and provide progress in the status message to the user.

Scope is permanent. Supported operation is Get.

<a href="" id="policyproviders"></a>**PolicyProviders**  
Required. Provides the settings that ESP reads during the Device Setup phase. Policy providers use this node to communicate progress state back to the ESP, which is then displayed to the user through progress message updates.

Scope is permanent. Supported operation is Get.

<a href="" id="policyproviders-providername"></a>**PolicyProviders**/***ProviderName***  
...
<a href="" id="policyproviders-providerid-trackingpoliciescreated"></a>**PolicyProviders/*ProviderID*/TrackingPoliciesCreated**  
Required. Indicates when the provider has created the required policies for the ESP to use for tracking app installation progress. The policy provider is expected to set the value of this node, not the MDM server.
